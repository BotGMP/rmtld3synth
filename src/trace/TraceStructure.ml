...
let to_number json =
  try to_float json with
  | Yojson.Basic.Util.Type_error _ -> float_of_int (to_int json)

let to_int_or_string json =
  try to_int json with
  | Yojson.Basic.Util.Type_error _ -> int_of_string (to_string json)

...
let create_event_expressions ctx trace =
  ...
  List.map (fun event ->
    ...
    let event_exprs_and_obbs = List.map (fun element ->
      ...
      | { region_type = "circle"; radius } ->
        ...
        Boolean.mk_const_s ctx (Printf.sprintf "a_%d" event_id), expr, obb_corners
    ...
    let combined_expr = mk_and ctx (List.map (fun (_, expr, _) -> expr) event_exprs_and_obbs) in
    let obbs = List.map (fun (_, _, obb) -> obb) event_exprs_and_obbs in
    (event_id, combined_expr, List.map (fun (z3_var, _, _) -> z3_var) event_exprs_and_obbs, obbs)
  ) trace.trace
...
let () =
  ...
  List.iter (fun json_file ->
    ...
    List.iter (fun (event_id, combined_expr, z3_vars, obbs) ->

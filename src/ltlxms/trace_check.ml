...
let run_ltlxms_trace_check ~trace_file ~property_file =
  ...
  let car_event_expressions =
    List.map (fun car_id ->
      (car_id, get_car_geometric_expressions ctx parsed_trace car_id x_var y_var)
    ) car_ids
  in

  List.iter (fun (car_id, exprs) ->
    if List.length exprs <> num_timestamps then
      failwith (Printf.sprintf "Could not extract expressions for all events for car %d" car_id)
  ...
  for t = 0 to num_timestamps - 2 do
    List.iter (fun (_car_id, exprs) ->
      let current_expr = List.nth exprs t in
      let next_actual_expr = List.nth exprs (t + 1) in
      let z3_symbolic_next = Formulas.Temporal.next ctx temporal_decls current_expr in
      let next_definition = Boolean.mk_iff ctx z3_symbolic_next next_actual_expr in
